...
describe("AlloyVault V2.1 contract", function () {
  ...
  before(async function () {
    ...
    hardhatVault = await vault.deploy(hardhatAlloyxTokenBronze.address,hardhatUsdcCoin.address,owner.address);
    goldFinchDelegacy = await ethers.getContractFactory("GoldfinchDelegacy");
    hardhatGoldfinchDelegacy = await goldFinchDelegacy.deploy(hardhatUsdcCoin.address,hardhatFiduCoin.address,hardhatGfiCoin.address,hardhatPoolTokens.address,hardhatSeniorPool.address,hardhatVault.address);
  ...
  describe("Basic Usecases", function () {
    ...
    it("Deposit NFT tokens:depositNFTToken", async function () {
      ...
      const prevPoolTokenBalance = await hardhatPoolTokens.balanceOf(hardhatGoldfinchDelegacy.address)
      await hardhatPoolTokens.mint([400,999],addr1.address)
      await hardhatUsdcCoin.mint(hardhatGoldfinchDelegacy.address,ethers.BigNumber.from(10).pow(6).mul(5))
      ...
      const postPoolTokenBalance =  await hardhatPoolTokens.balanceOf(hardhatGoldfinchDelegacy.address)
      expect(prevUSDC.sub(postUSDC)).to.equal(token5Value)
      expect(postPoolTokenValue).to.equal(ethers.BigNumber.from(10).pow(6).mul(5).add(prevPoolTokenValue))
    ...
    it("Purchase junior token:purchaseJuniorToken", async function () {
      ...
      await hardhatVault.approveDelegacy(hardhatUsdcCoin.address, hardhatTranchedPool.address,purchaseFee)
    ...
    it("Purchase senior token:purchaseSeniorTokens", async function () {
      ...
      await hardhatVault.approveDelegacy(hardhatUsdcCoin.address, hardhatSeniorPool.address,purchaseFee)
